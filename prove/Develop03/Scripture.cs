class Scripture
{
    ...
    public Scripture(string refrence, string text)
    { //constructor, creates word list and reference
        _scriptureText = text;
        CreateWords();
    ...
    public string GetScripture()
    {
        ...
        { // adds each word back (some might be cleared now)
    ...
    public void ClearRandomWords()
    {
        ...
        int i = 0;
        while (i < 3)
        { //removes 3 words (and keeps going till 3 are removed if duplicates are chosen)
            if (CheckIfAllWordsCleared())
            { // if less than 3 words are left, it will only remove the ones that are left
                break;
            }
            int x = _rnd.Next(0, _words.Count);
            if (_words[x].GetWordNotCleared())
            { //checks if a word is cleared
    ...
    { //kills program if all words are cleared
        bool check = true;
        foreach (Word word in _words)
        {
    ...
    { // Splits the scripture text into word strings, and makes Word instances out of them 
