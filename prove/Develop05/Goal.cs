...
//    private string _typeName;
    ...
    public Goal(string line){ //custom constructor if loading from a file
    ...
    public bool GetIsCompleted(){ //I ended up needing a ton of getters and setters in this assignment
        return _isCompleted;
    }
    ...
    public void SetPointValue(int pointValue){ // ^^ all getters and setters that I had to use
        _pointValue = pointValue;
    }
    public virtual void SetGoalData(){ // Gets user input for a regular goal
        _isCompleted = false;
    ...
    public virtual string SaveGoal(){ //standard save format
        return $"~{_goalName}|{_goalDiscription}|{_pointValue}|{_isCompleted}";
    }
    public virtual void DisplayGoal(){ // two cases for completed and not completed
        if(_isCompleted){
